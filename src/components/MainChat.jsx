...
console.log("sender",loginUserId ,selectedReceiverId)
  ...
  useEffect(() => {
    ...
    socket.emit('joinChat', {selectedReceiverId });
    // Emit joinChat to join a specific room between two users
    socket.emit('joinChat', { senderId: loginUserId, selectedReceiverId });

    // Listen for userJoined event from the backend
    socket.on('userJoined', (notification) => {
      console.log(notification); // e.g., "I'm has joined the room"
    });

    // Listen for message history for the room
...
console.log("messagesmessagesmessages",messages)
    ...
    const handleSendMessage = () => {
        if(selectedReceiverId){
            if (message.trim()) {
                // socket.emit('sendMessage', { senderId: loginUserId data  message });
                socket.emit('sendMessage', { senderId: loginUserId, selectedReceiverId, message });
                setMessage('');
              }
            ...
            <div className="main_chat overflow-auto">
                ...
                <div className="flex-1 p-4 overflow-auto">
                    {messages.map((message) => (
                        <div
                            key={message._id}
                            className={`flex ${message.senderId === loginUserId ? 'justify-end' : 'justify-start'} mb-4`}
                        >
                            ...
                            <div className={`${message.senderId === loginUserId ? 'bg-blue-500 text-white' : 'bg-gray-300 text-black'} p-3 rounded-xl flex items-end gap-2`}>
                                <Typography variant='body2' className='text-sm'>{message.message}</Typography>
                                {/* <p className="text-right text-xs">{message.time}</p> */}
                                {message.senderId === loginUserId && <DoneAllIcon className='w-4 h-4' />}
                            </div>
                        </div>
                    ))}
                </div>
